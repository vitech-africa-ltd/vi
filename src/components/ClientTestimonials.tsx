...
export const ClientTestimonials: React.FC = () => {
  ...
  const filteredList = useMemo(() => {
    ...
    return list;
  }, [testimonials, selectedCategory, selectedCountry]);

  const total = filteredList.length;

  // Reset index if category or country filter changes
  useEffect(() => {
    setCurrentIndex(0);
    setProgress(0);
  ...
  useEffect(() => {
    if (!isAutoPlaying || total <= 1 || viewMode === 'grid') return;
  ...
  }, [isAutoPlaying, total, currentIndex, viewMode]);
  ...
  if (!filteredList || filteredList.length === 0) {
    return (
      <section id="testimonials-section" className="py-24 bg-slate-900 dark:bg-slate-950 text-white text-center">
        <div className="max-w-7xl mx-auto px-4">
          <p className="text-slate-400">Aucun témoignage ne correspond à ces critères.</p>
          <button 
            onClick={() => { setSelectedCategory('all'); setSelectedCountry('all'); }}
            className="mt-4 px-4 py-2 bg-blue-600 rounded-xl text-xs font-bold"
          >
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          {/* Controls Bar: Country & Category Filters + View Mode Toggle */}
          <div className="space-y-3 mt-8">
            ...
            {/* Filter Category Pills & View Mode Switcher */}
            <div className="flex flex-wrap items-center justify-center gap-2 pt-2">
              ...
              >
                Tous Domaines ({testimonials.length})
              </button>
              ...
              {/* View Toggle */}
              <div className="flex items-center bg-slate-800 p-1 rounded-xl border border-slate-700 ml-1">
                <button
                  onClick={() => setViewMode('carousel')}
                  className={`p-1.5 rounded-lg text-xs transition-colors cursor-pointer ${viewMode === 'carousel' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  title="Vue Carrousel dynamique"
                >
        ...
        {viewMode === 'carousel' ? (
          ...
          /* 2. COMPLETE GRID VIEW MODE */
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in">
            {filteredList.map((item) => {
              const itemImpact = metricsMap[item.id] || { metric: '5.0 / 5', label: 'Qualité' };
              return (
                <div 
                  key={item.id}
                  className="bg-slate-950/80 rounded-3xl border border-slate-800 p-6 shadow-xl flex flex-col justify-between space-y-4 hover:border-slate-700 transition-all"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-950/80 border border-emerald-500/30 text-emerald-400 font-mono font-bold">
                        {itemImpact.metric}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-white flex items-center gap-1.5">
                          <span>{item.name}</span>
                          <span className="text-[10px] text-cyan-400 font-mono">({item.countryCode})</span>
                        </h4>
