          ...
          <div className="lg:col-span-8 text-left space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2">
              <button
                ...
                className="bg-[#1a44c2] hover:bg-[#1437a3] text-white px-8 py-3.5 rounded-full text-xs sm:text-sm font-extrabold uppercase tracking-wider shadow-xl shadow-blue-900/40 hover:shadow-2xl active:scale-95 transition-all inline-flex items-center gap-2 cursor-pointer"
              >
              ...
              <button
                ...
                className="bg-white/10 hover:bg-white/20 text-white border border-white/30 backdrop-blur-sm px-6 py-3.5 rounded-full text-xs sm:text-sm font-bold uppercase tracking-wider transition-all inline-flex items-center gap-2 cursor-pointer"
              >
              ...
              <button
                ...
                className="text-cyan-300 hover:text-white px-4 py-3.5 text-xs sm:text-sm font-bold tracking-wider uppercase inline-flex items-center gap-1.5 transition-colors cursor-pointer"
              >
            ...
            <div className="pt-6 border-t border-slate-800/80 grid grid-cols-2 sm:grid-cols-4 gap-4 text-left">
              <div>
                ...
                <div className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">{t('hero.statSla', 'Uptime SLA Garanti')}</div>
              </div>
              <div>
                <div className="text-xl sm:text-2xl font-black text-cyan-400 font-mono">{companyInfo.stats.projectsCompleted}</div>
                <div className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">{t('hero.statProjects', 'Projets Déployés')}</div>
              </div>
              <div>
                <div className="text-xl sm:text-2xl font-black text-amber-400 font-mono">{companyInfo.stats.countriesServed}</div>
                <div className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">{t('hero.statCountries', 'Pays Couverts')}</div>
              </div>
              <div>
                <div className="text-xl sm:text-2xl font-black text-emerald-400 font-mono">{companyInfo.stats.clientSatisfaction}</div>
                <div className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">{t('hero.statSatisfaction', 'Satisfaction Client')}</div>
              </div>
            </div>
