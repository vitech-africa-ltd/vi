...
export const ServicesSection: React.FC<ServicesSectionProps> = ({ onSelectServiceForQuote }) => {
  ...
  return (
    <section id="services" className="py-20 bg-slate-50 text-slate-900 relative overflow-hidden border-t border-slate-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-12">
          ...
          <div className="flex flex-wrap justify-center items-center gap-2 pt-4">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-full text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:border-blue-400 hover:bg-blue-50 transition-all flex items-center gap-1.5 cursor-pointer"
              title="Convertir les prix dans une autre devise"
            >
              <Coins className="w-3.5 h-3.5 text-blue-600" />
              <span>Tarifs en <strong className="text-blue-700 font-mono">{currencyOption.flag} {currencyOption.code}</strong></span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              return (
                <div
                  ...
                  className={`p-4 sm:p-5 rounded-2xl transition-all cursor-pointer border ${
                    isSelected
                      ? 'bg-white border-blue-600 shadow-xl ring-2 ring-blue-600/10'
                      : 'bg-white/80 hover:bg-white border-slate-200 hover:border-slate-300 shadow-xs'
                  }`}
                >
                  <div className="flex items-start gap-4">
                    <div className={`p-3 rounded-xl transition-colors ${
                      isSelected ? 'bg-[#1a44c2] text-white' : 'bg-slate-100 text-slate-700'
                    }`}>
          ...
          {currentService && (
            <div className="lg:col-span-7 bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xl space-y-6 sticky top-24">
              ...
              <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-6">
                ...
                <div className="text-right">
                  <div className="text-xs text-slate-500">{t('services.startingAt', 'À partir de')}</div>
                  <div className="text-2xl font-black text-[#1a44c2] font-mono">
              ...
              <div className="pt-4 border-t border-slate-100 space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(currentService.technologies || currentService.techStack || []).map((tech, idx) => (
                    <span
                      key={idx}
                      className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-800 text-xs font-mono font-medium border border-slate-200"
                    >
                      {tech}
                    </span>
                  ))}
